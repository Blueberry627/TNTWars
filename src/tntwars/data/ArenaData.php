...
class ArenaData
{
    ...
    /**
     * @param string $arena
     * @return string|null Returns 'red' if red team wins, 'blue' if blue team wins, or null if no winner.
     */
    public function getWinningTeam(string $arena): ?string
    {
        $redTeamPlayers = count($this->getPlayersRedTeam($arena));
        $blueTeamPlayers = count($this->getPlayersBlueTeam($arena));

        if ($redTeamPlayers === 0 && $blueTeamPlayers > 0) {
            return 'blue'; // Blue team wins
        } elseif ($blueTeamPlayers === 0 && $redTeamPlayers > 0) {
